...
router.post('/new-budget', async (req, res) => {
    try{
        ...
        const {Amount, BudgetDate, FromCategory, ToCategory, FromSubCategoryID, ToSubCategoryID} = req.body;
        const sqlQuery = `INSERT INTO budget (Amount, BudgetDate, FromCategory, ToCategory) VALUES (?, ?, ?, ?)`;

        const rows = await pool.query(sqlQuery, [Amount, BudgetDate, FromCategory, ToCategory]);
